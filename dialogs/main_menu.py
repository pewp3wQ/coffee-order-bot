...
async def start_order_dialog(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    username = callback.from_user.username if callback.from_user.username is not None else callback.from_user.full_name

    logger.info(f'Пользователь: {username} - ID: {callback.from_user.id} - начал свой заказ')

    conn = dialog_manager.middleware_data['conn']
    await add_user(conn, user_id=callback.from_user.id)
    order_id = await get_order_id(conn, user_id=callback.from_user.id, username=username)
